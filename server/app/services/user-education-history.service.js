...
exports.createUserEducationHistory = async (data) => {
    let major_subjects = [];
    data.major_subjects.map((value) => major_subjects.push({user_id: data.user_id, subject_id: value}));
    let minor_subjects = [];
    data.minor_subjects.map((value) => minor_subjects.push({user_id: data.user_id, subject_id: value}));
    const result = await sequelize.transaction(async (t) => {
        ...
        let response = await UserEducationHistoryModel.create(data, { transaction: t });
